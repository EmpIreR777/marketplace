...
import type { IPurchasedCoursesStoreModel } from '@/stores/Account/purchased-courses/models'
...
export const usePurchasedCoursesStore = defineStore('purchasedCoursesStore', {
  state: (): IPurchasedCoursesStoreModel => {
    return {
      renderKey: 0,
      infinteLoaderData: [],
      filtersCatalog: null,
  ...
  getters: {
    ...
    getFilteredTitle(state) {
      const { getPluralForms, pluralize } = usePluralize()
      const pluralizedCourses = pluralize(
        state.coursesCounter,
        getPluralForms(MainStatisticTitle[MainStatistic.COURSES]),
      ).toLowerCase()

      return ` ${state.coursesCounter} ${pluralizedCourses} ${state.coursesListType}`
    },
  },

  actions: {
    async loadPurchasedCoursesFilters() {
    ...
    pushInfiniteLoaderData(data: IPurchasedCourse[]) {
    ...
    async loadStore() {
      ...
      await this.loadPurchasedCoursesFilters()
    },
    ...
    updateFilteredTitle() {
      ...
      this.coursesCounter = state.count
      this.coursesListType = hasActiveFilters ? 'отфильтровано' : 'куплено'
    },
