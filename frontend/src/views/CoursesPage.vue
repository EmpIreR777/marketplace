<script setup lang="ts">
import { computed, onMounted, onUnmounted, ref } from 'vue'
import { useCoursesStore } from '@/stores/Courses'
import CoursesHeader from '@/components/courses/CoursesHeader.vue'
import { useDisplay } from 'vuetify'
import type { Recordable } from '@/types'
import CourseCard from '@/components/common/CourseCard.vue'
import FiltersPanel from '@/components/filters/FiltersPanel.vue'
import CoursesFilters from '@/components/courses/CoursesFilters.vue'
import SortingBar from '@/components/filters/SortingBar.vue'
import FilterTrigger from '@/components/filters/FilterTrigger.vue'
import { CoursesCategoryEnum, ORDERING_OPTIONS } from '@/enums/coursesFiltersEnum'
import BaseTextField from '@/components/common/BaseTextField.vue'

const coursesStore = useCoursesStore()
const { xs, width } = useDisplay()
const isMobile = computed(() => width.value < 767)
const filterListRef = ref()

const handleSortingUpdate = (val: string | null) => {
  coursesStore.setOrdering(val)
}

const handleVisibilityUpdate = (val: boolean) => {
  coursesStore.setFilterVisibility(val)
  if (isMobile.value) {
    if (val) {
      document.body.classList.add('fixed-scroll')
    } else {
      document.body.classList.remove('fixed-scroll')
    }
  }
}

const handleSearchInput = async (val: string) => {
  coursesStore.setSearch(val)
  await coursesStore.loadCoursesFilters()
}

const handleTopFiltersUpdate = async (val: CoursesCategoryEnum) => {
  coursesStore.setTopFilters(val)
  await coursesStore.loadCoursesFilters()
}

const handleFiltersUpdate = async (val: Recordable) => {
  coursesStore.setFilters(val)
  await coursesStore.updateFilters()
}

const handleFiltersReset = async () => {
  coursesStore.resetFilters()
  await coursesStore.updateFilters()
  // Clear filter inputs
  if (filterListRef.value) {
    filterListRef.value.checkListRef.forEach((checkList: any) => (checkList.search = ''))
    filterListRef.value.rangeListRef.forEach((rangeList: any) => rangeList.setRangeValues())
  }
}

onMounted(() => {
  coursesStore.loadStore()
})

onUnmounted(() => {
  coursesStore.clearStore()
})
</script>

<template>
  <div class="courses" :class="{ filter: coursesStore.isFiltersVisible }">
    <CoursesHeader
      title="Курсы"
      :search-input="coursesStore.search"
      :counters="coursesStore.topFiltersCount"
      @update-search="handleSearchInput"
      @update-filters="handleTopFiltersUpdate"
    />

    <div class="courses__header-filters">
      <BaseTextField
        v-if="!xs"
        v-model="coursesStore.search"
        label="Поиск"
        prepend-inner-icon="mdi-magnify"
        :debounce="300"
        clearable
        class="courses__header-search"
        @update:modelValue="handleSearchInput"
      />

      <div
        class="d-flex w-100 w-sm-auto align-center justify-space-between justify-sm-end pa-2 border-mobile ga-4"
      >
        <SortingBar
          :items="ORDERING_OPTIONS"
          :is-reversed="xs"
          class="order-1 order-sm-0"
          :ordering="coursesStore.ordering"
          @update-sorting="handleSortingUpdate"
        />
        <FilterTrigger
          :filter-visibility="coursesStore.isFiltersVisible"
          :is-reversed="xs"
          text="Фильтр"
          :count="coursesStore.getActiveFiltersCount"
          @update-visibility="handleVisibilityUpdate"
        />
      </div>
    </div>
    <div class="courses__main">
      <v-infinite-scroll
        :key="coursesStore.renderKey"
        :items="coursesStore.infinteLoaderData"
        :onLoad="coursesStore.loadOnScroll"
      >
        <div class="courses__grid">
          <CourseCard
            v-for="course in coursesStore.infinteLoaderData"
            :key="course.id"
            :course="course"
            class="courses__card"
          />
        </div>
        <template v-slot:loading>
          <div class="courses__loader-container">
            <v-progress-circular indeterminate />
          </div>
        </template>
        <template v-slot:empty></template>
      </v-infinite-scroll>

      <Teleport to="header" :disabled="!isMobile">
        <FiltersPanel
          v-if="coursesStore.isFiltersVisible"
          :is-mobile="isMobile"
          @update-visibility="handleVisibilityUpdate"
          @reset="handleFiltersReset"
        >
          <CoursesFilters
            v-if="coursesStore.getFiltersCatalog"
            ref="filterListRef"
            :filter-catalog="coursesStore.getFiltersCatalog"
            :filters="coursesStore.filters"
            :filters-update="handleFiltersUpdate"
          />
        </FiltersPanel>
      </Teleport>
    </div>
  </div>
</template>

<style scoped lang="scss">
.courses {
  display: flex;
  flex-direction: column;
  gap: 16px;

  &__header {
    display: flex;
    justify-content: space-between;
    align-items: center;
    padding: 12px 0;
    @include sm() {
      flex-wrap: wrap;
    }
    &-title {
      @include typography('h6');
      @include xs() {
        margin-bottom: 8px;
      }
    }

    &-filters {
      display: flex;
      justify-content: space-between;
      align-items: center;
      gap: 32px;

      @include xs() {
        flex-direction: column;
        gap: 16px;
      }

      &-sort {
        display: flex;
        align-items: center;
        gap: 8px;
      }

      & > button {
        padding: 0;
      }
    }

    &-search {
      max-width: 300px;

      @include xs() {
        max-width: 100%;
        width: 100%;
      }
    }
  }

  &__loader-container {
    display: flex;
    justify-content: center;
    align-items: center;
    padding: 12px;
  }

  & .v-infinite-scroll {
    overflow: hidden;
    width: 100%;
  }

  &.filter .v-infinite-scroll {
    width: calc(66.66% - (32px / 3));
    @include md {
      width: calc(50% - (16px / 3));
    }
    @include sm {
      width: calc(50% - 16px);
    }
  }

  & .v-infinite-scroll__side {
    padding: 0;
  }

  &__main {
    display: flex;
    justify-content: center;
    gap: 32px;
    width: 100%;

    @include md {
      gap: 16px;
    }

    & .filter-container {
      width: calc(33.33% - (32px / 1.5));

      @include md {
        width: calc(50% - (16px / 2));
      }
    }
  }

  &__grid {
    width: 100%;
    gap: 32px;
    display: flex;
    flex-wrap: wrap;

    @include md {
      gap: 16px;
    }

    & .courses__card {
      height: fit-content !important;
      width: calc(33.33% - (32px / 1.5));
      @include md {
        width: calc(50% - (16px / 2));
      }
      @include xs {
        width: 100%;
      }
    }
  }

  &.filter {
    & .courses__card {
      width: calc(50% - (32px / 2));
      @include md {
        width: 100%;
      }
    }
    @include xs {
      grid-template-columns: 1fr;
    }
  }

  &__filter {
    position: relative;
    transition: $transition;
    overflow: hidden;
    opacity: 0;
    width: 0;

    &.fade {
      opacity: 1;
      width: auto;
    }
  }
}
.border-mobile {
  @include xs {
    border-radius: 8px;
    border: 1px solid var(--Base-color-20, rgba(54, 57, 64, 0.2));
  }
}
</style>
